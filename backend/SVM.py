...
def make_predictions(list_inputs: list, vectorizer_file, model_file):
    X_test_v = preprocess_test_list(
        test_list=list_inputs, vectorizer_file=vectorizer_file
    )
...
if __name__ == "__main__":
    ...
    model_file = "loaded_models/tfidf_naive_bayes.pkl"
    predict = make_predictions(
        [
            "NÓNG! ZaloPay tung deal NỬA GIÁ. Nhập mã "
            "TINGDATA1"
            " để được giảm giá 50% (tối đa 50.000đ) khi mua gói data và thanh toán qua ZaloPay. Truy cập https://km.zalopay.vn/TSui/LUONGVEVIE để biết thông tin chi tiết về chương trình. LH 1900545436 (1.000đ/p). Từ chối QC, soạn TC7 VT_MALL gửi 197."
        ],
        vectorizer_file,
        model_file,
    )
